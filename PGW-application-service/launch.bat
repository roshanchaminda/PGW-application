...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "PGW-application-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name PGW-application-service apps/PGW-application-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app PGW-application-service:1.0-SNAPSHOT
		call oc expose svc/PGW-application-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd PGW-application-service
		cd target
