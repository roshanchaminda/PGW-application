...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "PGW-application-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name PGW-application-service apps/PGW-application-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app PGW-application-service:1.0-SNAPSHOT
    oc expose svc/PGW-application-service
else

	echo "Launching the application locally..."
	pattern="PGW-application-service"
	files=( $pattern )
	cd ${files[0]}
